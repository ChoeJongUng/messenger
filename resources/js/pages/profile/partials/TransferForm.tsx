      ...
      <div className="mt-6">
          <InputLabel
            ...
            <TextInput
              ...
              readOnly
            />
            {ebalance&&<p className="text-[red] text-[12px]">잔액이 충분하지 않습니다.</p>}
          ...
          <div className="mt-6">
            ...
            <Dropdown>
              <Dropdown.Trigger>
                <button className="btn btn-secondary flex items-center gap-2 w-full sm:w-3/4" onClick={(e)=>e.preventDefault()}>
              ...
              <Dropdown.Content>
                {friends.map((friend: { id:string;name:string })=>
                  (
                  <Dropdown.Button onClick={(e) => {e.preventDefault();setData("target_id", friend.id);setData("target_name",friend.name);setEtarget(false);}}>
                    {friend.name}
                  </Dropdown.Button>
                  )
                )}
            ...
            {etarget&&<p className="text-[red] text-[12px]">받을사람을 선택하십시오.</p>}
          ...
          <div className="mt-6">
            ...
            <TextInput
              ...
              name="amount"
              value={data.amount}
              onChange={(e) => {setData("amount", parseInt(e.target.value));setEamount(false);setEbalance(false);}}
              className="mt-1 block w-full sm:w-3/4"
              placeholder="amount"
            />
            
            {eamount&&<p className="text-[red] text-[12px]">이채금액을 정확히 입력하십시오.</p>}
            <InputError message={errors.amount} className="mt-2" />
          </div>

        </form>
      <DangerButton onClick={confirmUserDeletion} className="w-full bg-[forestgreen]">이채</DangerButton>
